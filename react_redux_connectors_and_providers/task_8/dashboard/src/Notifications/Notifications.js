...
export const mapStateToProps = (state) => {
	return {
		...
		listUnreadNotification: getUnreadNotificationsByType(state),
	};
};
          ...
          <div data-testid='menuItem' className={css(styles.menuItem)} onClick={handleDisplayDrawer}>
			...
			{displayDrawer && (
				<div className={css(styles.notifications)} data-testid="notifications">
					{listUnreadNotification && listUnreadNotification.size > 0 && (
						<p className={css(styles.text)}>Here is the list of notifications</p>
					)}
					<button data-testid='urgent-button' onClick={() => { handleFilterUrgent(); }} className={css(styles.filterButton)} aria-label='urgent'>!!</button>
					<button data-testid='default-button' onClick={() => { handleFilterDefault(); }} className={css(styles.filterButton)} aria-label='default'>?</button>
